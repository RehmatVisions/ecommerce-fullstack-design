...
const HomeGadgets = () => {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      ...
      // Show up to 8 products for home gadgets section
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        >
          ...
          <button 
            ...
            className="bg-white text-black px-3 sm:px-4 py-2 rounded text-xs sm:text-sm hover:bg-gray-100 hover:shadow-md transition-all duration-300"
          >
            Source Now
          </button>
        ...
        {products.map((product, index) => (
          ...
          >
            ...
            <Link to={`/product/${product._id || product.id}`} className="flex-1">
              <div className="w-full h-24 sm:h-32 flex items-center justify-center mb-2 overflow-hidden">
                <img
                  src={product.image || product.imageUrl}
                  alt={product.name || `Home gadget ${index + 1}`}
